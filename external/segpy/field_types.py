...
class IntFieldMeta(type):
    ...
    def class_new(cls, *args, **kwargs):
        ...
        if not (cls.MINIMUM <= instance <= cls.MAXIMUM):
            raise ValueError(
                "{} value {!r} outside range {} to {}".format(
                    cls.__name__, instance, cls.MINIMUM, cls.MAXIMUM
                )
            )
        return instance
...
class Int16(int, metaclass=IntFieldMeta, seg_y_type="int16"):
    """16-bit signed integer."""

    pass


class NNInt16(int, metaclass=IntFieldMeta, seg_y_type="nnint16"):
    """Non-negative 16-bit signed integer."""

    pass


class Int32(int, metaclass=IntFieldMeta, seg_y_type="int32"):
    """32-bit signed integer."""

    pass


class NNInt32(int, metaclass=IntFieldMeta, seg_y_type="nnint32"):
...
class IntEnumFieldMeta(IntFieldMeta):
    ...
    def __new__(cls, name, bases, namespace, enum, seg_y_type="int16", **kwargs):
        if any(
            (value < LIMITS[seg_y_type].min) or (value > LIMITS[seg_y_type].max)
            for value in enum
        ):
            raise ValueError(
