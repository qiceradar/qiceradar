...
from pyfive.btree import BTreeV1Groups
...
class DataObjects(object):
    ...
    def _parse_v2_header(fh):
        ...
        header["size_of_chunk_0"] = struct.unpack(
            chunk_fmt, fh.read(struct.calcsize(chunk_fmt))
        )[0]
    ...
    def _parse_attribute_msg(self, buffer, offset):
        ...
        try:
            ...
            warnings.warn(f"Attribute {name} type not implemented, set to None.")
            return name, None
    ...
    def fillvalue(self):
        ...
        if version == 1 or version == 2:
            ...
            raise InvalidHDF5File("Unknown fillvalue msg version:" + str(version))
    ...
    def compression_opts(self):
        ...
        if GZIP_DEFLATE_FILTER in self._filter_ids:
            gzip_entry = [
                d for d in self.filter_pipeline if d["filter_id"] == GZIP_DEFLATE_FILTER
            ][0]
    ...
    def filter_pipeline(self):
        ...
        if version == 1:
            ...
            for _ in range(nfilters):
                ...
                if name_length > 0:
                    name = struct.unpack_from(
                        "{:d}s".format(name_length), self.msg_data, offset
                    )[0]
                    offset += name_length
                filter_info["name"] = name
                client_values = struct.unpack_from(
                    "<{:d}i".format(num_client_values), self.msg_data, offset
                )
            ...
            raise NotImplementedError(
                "filter pipeline description version > 2 is not supported"
            )
    ...
    def _get_chunk_params(self):
        ...
        if (version == 1) or (version == 2):
            ...
            dims, address = struct.unpack_from("<BQ", self.msg_data, property_offset)
        ...
        if not supported_version:
            ...
            raise RuntimeError(
                f"Pyfive cannot yet read HDF5 files with layout class {version}"
            )
    ...
    def _iter_links_from_symbol_tables(self, sym_tbl_msg):
        ...
        data = _unpack_struct_from(
            SYMBOL_TABLE_MSG, self.msg_data, sym_tbl_msg["offset_to_message"]
        )
        yield from self._iter_links_btree_v1(
            data["btree_address"], data["heap_address"]
        )
    ...
    def _iter_link_from_link_info_msg(self, info_msg):
        ...
        if name_btree_address:
            yield from self._iter_links_btree_v2(
                name_btree_address, order_btree_address, heap_address
            )

    def _iter_links_btree_v2(
        self, name_btree_address, order_btree_address, heap_address
    ):
        """Retrieve links from symbol table message."""
        heap = FractalHeap(self.fh, heap_address)
        ordered = order_btree_address is not None
    ...
    def _get_data_message_properties(self, msg_offset):
        ...
        version, arg1, arg2 = struct.unpack_from("<BBB", self.msg_data, msg_offset)
        if (version == 1) or (version == 2):
            dims = arg1
            layout_class = arg2
