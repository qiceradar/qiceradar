...
class DatatypeMessage(object):
    ...
    def _determine_dtype_compound(self, datatype_msg):
        ...
        for _ in range(n_comp):
            ...
            name_size = (
                null_location - self.offset + 1
                if version == 3
                else _padded_size(null_location - self.offset + 1, 8)
            )
            ...
            fields.append(
                P5CompoundField(name=name, offset=prop_desc["offset"], ptype=comp_dtype)
            )
    ...
    def _determine_dtype_enum(self, datatype_msg):
        ...
        enum_msg = _unpack_struct_from(
            ENUM_DATATYPE_MSG, self.buf, self.offset - DATATYPE_MSG_SIZE
        )
